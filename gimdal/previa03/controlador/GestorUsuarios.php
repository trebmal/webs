...
class GestorUsuarios {
    ...
    public function insertarUsuario($usuario){
        ...
        $consulta = sprintf("INSERT INTO usuarios (id_usuaris, nombre, apellidos, password) VALUES ('%s','%s','%s','%s')",        
     ...
     * @return boolean 
     */
    public function existeUsuario($id_usuaris){
        $consulta = sprintf("SELECT id_usuaris FROM usuarios WHERE id_usuaris='%s'",
            $this->mysqli->real_escape_string($id_usuaris)
        );
     ...
     * @return boolean 
     */
    public function comprobarUsuario($id_usuaris,$pass){
        $consulta = sprintf("SELECT id_usuaris, password FROM usuarios WHERE id_usuaris='%s' AND password='%s'",
            $this->mysqli->real_escape_string($id_usuaris),
            $this->mysqli->real_escape_string(md5(sha1($pass).$id_usuaris))
        );
        $result = $this->mysqli->query($consulta);
